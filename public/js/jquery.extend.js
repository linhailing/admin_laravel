...
(function($) {
    ...
    $.fn.selectBox = function (data, val, isgroup, title) {
        ...
        ///	 数据列表 [{ id = "1", name = "1", node = [{ id = "11", name = "11", node = []}, { id = "12", name = "12", node = []}]}]
        /// </param>
        ...
        var html = '<option value="-1">' + (title || "请选择") + '</option>';
    ...
    $.fn.asyncSelectBox = function (data, val, isgroup, title, fun) {
        ...
        isgroup = typeof isgroup == 'boolean' ? isgroup : false;
        sel.append('<option value="-1">-' + (title || "请选择") + '-</option>');
    ...
    $.fn.checkTextData = function (output, min, max, title, submit, isNVarchar) {
        ...
        /// <param name="isNVarchar" type="bool">是否NVarchar類型的数据</param>
        var obj = this;
        var len = (isNVarchar) ? obj.val().length : obj.val().lengthEx();
        title = title.split('：')[0].split(':')[0];
        if (len == 0 && min != 0) {
            if (output != "") {
                output.remove("*").notice(title + "不能为空", "error");
            ...
            if (output != "") {
                output.remove("*").notice(title + "不能少于" + min + "位", "error");
                try { if (submit) { obj.focus(); }; } catch (e) { };
            } else {
                $(document).alert({
                    id: "sysalert",
                    text: title + "不能少于" + min + "位",
                    cancelClick: function () { if (submit) { obj.focus(); } }
                });
            };
            return false;
            ...
            if (output != "") {
                output.remove("*").notice(title + "不能大于" + max + "位", "error");
                try { if (submit) { obj.focus(); }; } catch (e) { };
            } else {
                $(document).alert({
                    id: "sysalert",
                    text: title + "不能大于" + max + "位",
                    cancelClick: function () { if (submit) { obj.focus(); } }
                });
            };
            return false;
    ...
    $.fn.checkSelectData = function (output, title, submit) {
        ...
        if (value <= 0) {
            if (output != "") {
                output.remove("*").notice("请选择" + title + "！", "error");
                try { if (submit) { obj.focus(); }; } catch (e) { };
            } else {
                $(document).alert({
                    id: "sysalert",
    ...
    $.fn.alert = function (options) {
        ...
        /// </param>
        if (!options) options = {};
        var html = $.operatorTemplate.replace("取消", options.cancelTitle || "关闭").format('<div class="left alert">&nbsp;</div><div class="text">{0}</div>'.format(options.text || "无提示"), 'icon', '');
    ...
    $.fn.popupProcess = function (options) {
        ...
        /// </param>
        if (!options) options = { };
        var html = '<div class="processMsg" style="border:0px;background-color:#fff;margin:5px; 0px;">{0}</div>'.format(options.text || "正在处理数据，请等待...");
        if (!options.html) options.html = html;
    ...
    $.fn.popupLoading = function (options) {
        ...
        /// </param>
        if (!options) options = { };
        if (!options.text) options.text = "正在加载数据...";
    ...
    $.fn.notice = function (options) {
        ...
        var defaultOptions = {
            ...
            classEx: "",			//附加樣式類
            styleEx: "",			//附加樣式
            isTop: false			//true 加载到最上面false 加载到最下面
        };
        var opts = $.extend({}, defaultOptions, options);
    ...
    $.fn.process = function (options) {
        ...
        /// </param>
        if (!options) options = {};
        if (!options.text) options.text = "正在处理数据，请等待...";
